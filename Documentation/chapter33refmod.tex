...
%----------------------------------------------------------------------------
	...
	A párhuzamosítást a CPU-n fõként a legbelsõ, gyorsulás értékeket kiszámító \verb+calculateAcceleration+ függvény módosításával értem el. A megvalósított képlet mindig az \sectref{NBody_integrateSect} fejezetben bemutatott \eqref{NBody_accEq} összefüggés.

	Az alábbi kódrészlet a teljesen natív C++ megvalósítás:
	
	\begin{lstlisting}
float3 NBodyAlgorithmCPU::calculateAcceleration(
const float3 posI, 	 // A test, melynek a gyorsulás értékének egy darabját számoljuk
...
const float3 posJ) { // A másik test pozíciója
	...
	// A két test skaláris távolsága belekalkulálva az ún. softening factor-t
	...
	Az alábbi kódrészleteket az egyik elsõ verzióból való azok közül, amit írtam. A többi egy kicsit jobban feldarabolt a párhuzamosítás miatt. Viszont az alapelv változatlan és ebben a formában könnyebben értelmezhetõ.

	A testeket egy \verb+vector+ konténerben tárolom, ami a dinamikus tömbkezelés problémáit (allokáció, reallokáció, felszabadítás) nehézségeit veszi le a fejlesztõk válláról.
...
bodies.at(i).acceleration.x // az i. test x irányú gyorsulás értéke (float)
	...
	A már említett \verb+advance+ függvény belsejében található kettõ, egymásba ágyazott for ciklus. A külsõ ciklus mindig az éppen gyorsulás szempontjából frissítendõ testet jelöli ki, míg a belsõ a párját. A ciklusmag mindig az elõzõ szimulációs iterációban kiszámolt gyorsulás értékek kinullázásával kezdõdik.
...
void NBodyAlgorithmCPUAllPairs::advance(std::vector<Body> &bodies) {
	...
	Annak érdekében, hogy az egyes gyorsulás értékek, konzisztens pozícióadatokból legyen kiszámítva, az integrálás (\eqref{NBody_integrateEq} képlet) végrehajtását egy másik ciklusban kell elvégezni. Összesen $N$-szer végrehajtani a ciklust, iterációnként ez 21 FLOP-ot jelent. Ebbõl 3 FLOP-ot, hoz be egy gyorsulás értékét csökkentõ plusz konstans, melynek feladata, hogy a szimuláció során ne hagyja a testeket egymástól nagyon eltávolodni. A grafikus megjelenítés rögzített, nem mozdítható vagy forgatható és gyakran jönnek, mennek az ablak szélén a testek.
