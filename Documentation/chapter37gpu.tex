...
%----------------------------------------------------------------------------
	...
	%----------------------------------------------------------------------------
		...
		Az ábrán beszürkített $4\cdot4$-es tile esetén összesen 8 testhez tartozó információ egyszeri felhozatala szükséges, a 16 helyett, ha az adatokat el lehetne tárolni a blokk által közösen használ \emph{shared memóriában}. A szálak a számítást már csak akkor kezdené el, amikor minden test adata már betöltõdött oda.
	...
	%----------------------------------------------------------------------------
		...
		Minden TB egy dimenziós lesz pontosan $P$ darab szállal (ez pontosan a $blockDim.x$ beépített változó értékét fogja meghatározni). Minden szál betölti a regisztereibe azt az egy testet, amely minden számítás során dolgozni fog. A saját beépített $threadIdx.x$ és az éppen aktuális tile sorszáma alapján betölt egy darab testet a shared memóriába. Ha jól van eltárolva az adat a globális memóriában, akkor egy warp (32 szál) képes felhozni egy olvasással $32$ \verb+float+ adatot és eltárolni azt bank konfliktus nélkül a shared memóriába.
	...
	%----------------------------------------------------------------------------
		...
		Elõször szépen \verb+float3+-mal próbálkoztam a pozíció értékek és \verb+float+-tal a tömegek eltárolására, de egy jobb megoldásnak bizonyul, ha \verb+float4+-et alkalmazok. Ennek oka lehet, hogy globális memóriában a \verb+float4+-ek kitöltenek egy teljes szegmenst, míg a \verb+float3+-ak nem. Ezért \verb+float3+-nál több szegmens kiolvasása történhet meg, ha a szükséges adat két szegmensbe is belóg. A \verb+float4+ negyedik mezõjében (w) került eltárolásra a tömeg.
...
__device__ float3 advanceWithFloat4(float4 posI, float4 *g_pos) {
	...
	return accI;
		...
		Shared memória megfelelõ használatának egy fontos eleme a bankkonfliktus elkerülése, mely során biztosítjuk, hogy a warp-on belüli szálak, más-más bank 32 bites rekeszeihez férünk hozzá. Ez a fenti kódban a \verb+float4+ használata miatt sajnos nem teljesül.
