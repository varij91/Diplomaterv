...
%----------------------------------------------------------------------------
	...
	%----------------------------------------------------------------------------
		...
		Az elsõ probléma, ami szembe jön, az a memória hozzáférések száma. Ha minden szál csak egy testpár közötti gyorsulást számol, akkor egy testet összes ki kell olvasni a globális memóriából $(2N-2)$-ször. Egy test $N$-szer célpont, és $N$-szer hat másik testre. A $(-2)$ pedig az önmagával vett interkaciók miatt jön be. És bár rendelkezésre áll valamennyi cache, a tranzakciók száma így is nagy, ami hátráltatni fogja a számításokat és a korlátot nem a számítási kapacitás fogja jelenteni, hanem a memória sávszélessége.
	...
	%----------------------------------------------------------------------------
		...
		Elõször szépen \verb+float3+-mal próbálkoztam a pozícióértékek és \verb+float+-tal a tömegek eltárolására, de egy jobb megoldásnak bizonyul, ha \verb+float4+-et alkalmazok. Ennek oka lehet, hogy globális memóriában a \verb+float4+-ek kitöltenek egy teljes szegmenst, míg a \verb+float3+-ak nem. Ezért \verb+float3+-nál több szegmens kiolvasása történhet meg, ha a szükséges adat két szegmensbe is belóg. A \verb+float4+ negyedik mezõjében (w) került eltárolásra a tömeg.
